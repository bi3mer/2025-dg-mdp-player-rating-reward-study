...
with open(os.path.join("..", "data", "level_to_stats.csv"), "r") as f:
    ...
    for line in f.readlines():
        ...
        level_to_stats[f"{id}_0"] = Stat(d, e)
...
# Rest of the nodes
terminal_levels = []
for level_id in level_to_stats:
    is_terminal = True
    ...
    s = level_to_stats[level_id]

    if is_terminal:
        print(f"{level_id} is terminal :/")
        terminal_levels.append(level_id)
    else:
        typescript += f"MDP.addNode(new CustomNode('{level_id}', {s.difficulty}, {s.enjoyability}, 0, false, [], -1));\n"
